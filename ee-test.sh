...
rm -f bundle.zip
bundle_id=$$
curl --insecure -XPUT -H "Authorization: token=$(dcos config show core.dcos_acs_token)" https://172.28.128.31:61002/system/health/v1/node/diagnostics/${bundle_id}
status=$(curl --insecure -H "Authorization: token=$(dcos config show core.dcos_acs_token)" https://172.28.128.31:61002/system/health/v1/node/diagnostics/${bundle_id} | jq '.status')
while [ $status != '"Done"' ]
do
	echo $status
	sleep 5
...
# Port 61001 fails on strict clusters
set +e
